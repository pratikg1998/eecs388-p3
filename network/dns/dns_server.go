...
Initializes the DNS server's cache, and then listens on UDP port 8090
...
func main() {
	...
	connection, _ := net.ListenUDP("udp", &net.UDPAddr{Port: 53})
	...
	for {
		...
		_, clientAddress, _ := connection.ReadFrom(buffer)
